...
{
    public class DatabaseTableTypedSimpleIndexed<KeyType, IndexKeyType, ItemType> : DatabaseTableTyped<KeyType, ItemType>
        where ItemType : DatabaseItem, new()
        where IndexKeyType : IBaseObject, new()
    {
        public DatabaseTableTypedSimpleIndexed(string name, string databaseDirectory, bool generateKeys,
                LanguageID languageID, bool caseInsensitive)
            : base(name, databaseDirectory, generateKeys, languageID, caseInsensitive)
        {
        }

        public DatabaseTableTypedSimpleIndexed()
        {
        }

        public override DatabaseMatcher CreateDatabaseMatcher(Matcher subMatcher)
        {
            return new DatabaseFieldOnlyMatcher(this, subMatcher);
        }
        ...
        public override bool Contains(Matcher matcher)
        {
            ...
            else
            {
                ...
                var items = _DB.Table<ItemType>().IndexQuery<IndexKeyType>("Index").Where(i => dbMatcher.Match(i)).ToLazyList();
                if (items.Count() != 0)
                    returnValue = true;
            }
        ...
        public override IBaseObject GetFirst(Matcher matcher)
        {
            ...
            {
                DatabaseMatcher dbMatcher = CreateDatabaseMatcher(matcher);
                var lazyItems = _DB.Table<ItemType>().IndexQuery<IndexKeyType>("Index").Where(i => dbMatcher.Match(i)).ToLazyList();
        ...
        public override List<object> QueryKeys(Matcher matcher)
        {
            ...
            {
                DatabaseMatcher dbMatcher = CreateDatabaseMatcher(matcher);
                items = _DB.Table<ItemType>().IndexQuery<IndexKeyType>("Index").Where(i => dbMatcher.Match(i)).ToList();
            }
